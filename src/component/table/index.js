...
import { InvestorDatosItem, InvestorItem , FundItem, ProjectItem, RiskItem, BillingItem, BillingDetailItem, BankConfigItem, ContractManagementItem } from './InvestorTable';
...
export const ContractManagementTable = ({header1,header2,header3,header4,header5,data,link,currentPage,setCurrentPage}) => { 
    const [tableItems, setTableItems] = useState([]);
    const [loading, setLoading] = useState(false);
    const [itemsPerPage] = useState(5);
    const [currentItems, setCurrentItems] = useState([]);
        ...
        <div className="table-responsive">
            <div className="table-lists ContractManagement">
                ...
                </ul>
                {
                    currentItems.map((info,i)=>{
                        return(
                            <ContractManagementItem key={i} info={info} link={link} />
                        )    
                    })
                }
            </div>
