...
const NewWar = ({ title ,language}) => {
    ...
    return ( <>
        ...
        <div className = "adding-investor NewWar">
            <div className = "investor-add">
                <div className = "header-add">
                    <ButtonWithArrow text = { language.global.back }background = { `var(--primary-color)` }/> 
                    <h1>{title}</h1>
                </div> 
                <form className='NewWarForm' action="/Projects" method='POST' onSubmit={handleSubmit}>
                    <div className="NewWar-body">
                        <ul>
