...
const DebtDetail = ({lang, setLang, language, responsive}) => {
  ...
  return (
    <>
        <div className="leasingConfig">
            ...
            <Header responsive={responsive} lang={lang} setLang={setLang} displayArrowBtn={`show`} colorArrowBtn={`var(--primary-color)`} textArrowBtn={language.global.back}  pageTitle={`Debt contract detail  leasing`} pageDesc ={`Administration and management`} displaySearch={`show`} />
            <main className='main configuration'>
                <Tab active={funcion} action={setTab2} tab1={`Contract creation`} tab2={`Approval/ Denial`} tab3={`Document Generate`} tab4={`Generation of invoices`} hideTab5={`dn`} hideTab6={`dn`} hideTab7={`dn`} hideCustomizer={`dn`} tabs1={language.deuda_leasing.tab1} tabs2={language.deuda_leasing.tab2} tabs3={`Document Generate`} tabs4={`Generation of invoices`} />
                {
