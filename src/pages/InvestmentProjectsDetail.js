...
const ProjectDetail = ({lang, setLang, language, responsive}) => {
  ...
  return (
    <>
        <div className="ManageProject">
            ...
            <Header responsive={responsive} lang={lang} setLang={setLang} displayArrowBtn={`show`} colorArrowBtn={`var(--primary-color)`} textArrowBtn={language.global.back}  pageTitle={`Detail of investment contract to projects`} pageDesc={`Administration and management`} displaySearch={`show`} />
            <main className='main configuration'>
                <Tab active={funcion} action={setTab2} tab1={`Contract creation`} tab2={`Approval/ Denial`} tab3={`Document Generate`} hideTab4={`dn`} hideTab5={`dn`} hideTab6={`dn`} hideTab7={`dn`} hideCustomizer={`dn`} tabs1={language.deuda_leasing.tab1} tabs2={language.deuda_leasing.tab2} tabs3={`Document Generate`} />
                {
                    ...
                    ?<ContractCreationProject language={language} valorExtraido={valores} valorIdProyecto={idProjectDeu} valorIdFondo={idFondo} valorIdOwner={idOwner} Deuda={deud}  isUpdates={isUpdate} funcion={funcioGloal} edit={false} />
                    :<></>
                }
                {
                    tab2 === 'Approval/ Denial' 
                    ?<ApprovalDenalProject language={language} AprovalInf={AprovalInfo} idFondos={idFondo} idproyecto={idProjectDeu} valoridOwner={idOwner} Deuda={deud} edit={false} />
                    :<></>
                }
                {
