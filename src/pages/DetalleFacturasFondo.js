...
const BillingDetail = ({responsive, lang, setLang, language}) => {
        ...
        if(invoice){
          invoice.forEach((voice)=>{
            voice.addEventListener("click",()=>{
              console.log(voice.dataset.state)
  ...
  return (
    <>
      {
        inVoice === "open"
        ?<Invoice language={language} idinvoce={idInvoce}  state={language.billing_detail.invoice_detanotyet} color={`#85B900`} />
        :<></>
      }
      {
        expire === "open"
        ?<Invoice language={language} idinvoce={idInvoce}  state={language.billing_detail.invoice_detaexp} color={`#DE5753`} />
        :<></>
      }
      <div className="Billing">
        ...
        <main className="main billing billing-deatil">
            ...
            <BillingDetailTable header1={language.billing_detail.header1}  header2={language.billing_detail.header2}  header3={language.billing_detail.header3}  header4={language.billing_detail.header4} header5={language.billing_detail.header5}  header6={language.billing_detail.header6} data={invoice} currentPage={currentPage} setCurrentPage={setCurrentPage} />
        </main>
      </div>
    </>
  )
